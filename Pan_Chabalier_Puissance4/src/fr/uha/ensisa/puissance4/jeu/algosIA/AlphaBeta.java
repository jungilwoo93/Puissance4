...
public class AlphaBeta extends Algorithm {
	...
	private double minValue(Grille grille, int profondeur, int tourSimule) {
		if (profondeur == 0 || grille.getEtatPartie(symboleMin,
				tourDepart + profondeur + tourSimule) != Constantes.PARTIE_EN_COURS) {
	...
	/**
	 * permet de retourner la valeur maximale des noeuds inferieurs. On essaye ici
	 * de maximiser le jeu du joueur On elague l'arbre de jeu selon l'algorithme
	 * alpha-beta
	 * 
	 * @param grille
	 *            grille simulée. Reflète la grille de depart et les reflexions
	...
	private double maxValue(Grille grille, int profondeur, int tourSimule) {
		if (profondeur == 0 || grille.getEtatPartie(symboleMax,
				tourDepart + profondeur + tourSimule) != Constantes.PARTIE_EN_COURS) {
